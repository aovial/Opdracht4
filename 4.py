...
#wk22= pd.read_csv('wk20222.csv')
att = pd.read_csv('attendance.csv')
att = pd.DataFrame(att)
#wk22= pd.DataFrame(wk22)
...
if pages == 'Home':
    ...
    st.subheader("Hier worden de visualisaties weergegeven die wij hebben opgesteld."), st.markdown('De onderstaande figuur laat het aantal deelnemers per WK zien. Het aantal teams is sinds 1930 meer dan verdubbeld. In 1942 & 1946 is er geen WK gespeeld door de Tweede Wereldoorlog.'), st.plotly_chart(fig1), st.markdown('In de onderstaande kaart worden de deelnemers per WK weergegeven. Druk op een land op het behaalde aantal punten te zien.') ,folium_static(c_map), st.markdown('De onderstaande figuur laat het aantal doelpunten en deelnemers per WK zien. De toename in teams verklaart de stijging in het totale aantal doelpunten.') ,st.plotly_chart(fig2), st.markdown('In het lijndiagram hieronder wordt het gemiddelde aantal doelpunten per WK weergegeven.'),st.plotly_chart(fig3), st.markdown('Het onderstaande lijndiagram geeft het totale aantal toeschouwers per WK weer. Ook hier is de groei van het WK duidelijk zichtbaar. De piek in 1994(WK VS) komt door de grootte van de gebruikte stadions.'), st.plotly_chart(fig4), st.markdown('De regressielijn lijn in de onderstaande grafiek laat de toename van het aantal toeschouwers zien. Voor het aankomende WK in Qatar worden ruim 3.8 miljoen toeschouwers verwacht.'), st.plotly_chart(fig5), st.markdown('De cirkeldiagrammen hieronder geven het winstpercentage van de thuisploegen op de WKs van 2010, 2014 en 2018. Een duidelijk verschil is alleen in 2014 zichtbaar.') ,st.plotly_chart(fig6), st.plotly_chart(fig7), st.plotly_chart(fig8), st.markdown('In het spreidingsdiagram hieronder is het resultaat van de thuisploeg ten opzichte van de plaats op de FIFA ranglijst van beide teams te zien.'),st.plotly_chart(fig9), st.markdown('De onderstaande kaart geeft de deelnemende landen voor het aankomende WK in Qatar weer, ook is te zien in welke poule de landen zij ingedeeld.') ,folium_static(o_map)
elif pages == 'Einde':
    st.markdown('Bedankt voor het bezoeken.')
    st.markdown('Groep 22: Bente van Hameren, Lukas Čović.')
    st.markdown("![Alt Text](https://media.giphy.com/media/9P1wgP4XPGhBnwltWq/giphy-downsized.gif)")
